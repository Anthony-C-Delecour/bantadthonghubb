...
import { Star, Clock, MapPin, Navigation, X } from "lucide-react";
...
export function BantadthongMap({
  ...
  useEffect(() => {
    if (!userLocation && navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        (pos) => {
          // Check if user is in Bangkok area, otherwise use default Bantadthong location
          const lat = pos.coords.latitude;
          const lng = pos.coords.longitude;
          if (lat > 13.5 && lat < 14.0 && lng > 100.3 && lng < 100.8) {
            setUserPos({ lat, lng });
          } else {
            // Default to Bantadthong area for demo
  ...
  // Calculate simple route (straight line for demo - in production use OSRM)
